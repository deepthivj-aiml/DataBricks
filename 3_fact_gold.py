...
# DBTITLE 1,Cell 5
# Cast string columns to numeric for arithmetic (try_cast tolerates malformed values like "Two")
df = df.withColumn("quantity", F.expr("try_cast(quantity AS FLOAT)")) \
       .withColumn("unit_price", F.expr("try_cast(regexp_replace(unit_price, '[$,]', '') AS FLOAT)")) \
       .withColumn("discount_pct", F.expr("try_cast(regexp_replace(discount_pct, '%', '') AS FLOAT)")) \
       .withColumn("tax_amount", F.expr("try_cast(tax_amount AS FLOAT)"))
